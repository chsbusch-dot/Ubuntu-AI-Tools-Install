...
#   5. Bats unit tests (tests/*.bats)
#   6. Kcov coverage (opt-in via --coverage)
#   7. Ollama model name validation (network)
#   8. HuggingFace repo validation (network)
#
# Usage:
...
fail()  { echo -e "  ${RED}❌ $1${RESET}"; TOTAL_ERRORS=$((TOTAL_ERRORS + 1)); }
warn()  { echo -e "  ${YELLOW}⚠️  $1${RESET}"; TOTAL_WARNINGS=$((TOTAL_WARNINGS + 1)); }
header(){ echo -e "\n${BLUE}=== $1 ===${RESET}"; }
...
# ─── 6. Kcov Coverage (opt-in) ───────────────────────────────────
if [ "$RUN_COVERAGE" = true ]; then
    header "6. Kcov Coverage"
...
# ─── 7. Model Name Validation (network) ──────────────────────────
if [ "$QUICK_MODE" = true ]; then
    header "7. Model Name Validation (SKIPPED — quick mode)"
    echo "  Run without --quick to check Ollama + HuggingFace repos over the network."
else
    header "7. Ollama Model Validation (network)"
...
$REC_MODEL_VISION"
    ...
    header "8. HuggingFace Repo Validation (network)"
